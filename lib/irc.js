...
, 472: { name: "ERR_UNKNOWNMODE"
       , template: "<%= mode %> :is unknown mode char to me" }
...
, 215: { name: "RPL_STATSILINE"
       , template: "I <%= host %> * <%= host2 %> <%= port %> <%= cls %>" }
...
, 242: { name: "RPL_STATSUPTIME"
       , template: ":Server Up <%= days %> days <%= hours %>:<%= minutes %>:<%= seconds %>" }
...
Object.keys(replies).forEach(function(code) {
  ...
  reply.template = exports.template(reply.template);
...
Client.prototype.reply = function(code, context) {
  ...
  if (reply) {
    ...
    this.write(this.server.host, code, [this.nick, reply.template(context)]);
  } else {
